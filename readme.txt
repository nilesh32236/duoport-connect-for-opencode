...
By default only allowlisted chat/completions models are shown (Go: 38, Zen: 71 including free models). Enable **Show all models** on Settings → DuoPort Connector to expose every model from the API (including non-chat models that may fail).
...
= 0.1.3 =
* Updated: refreshed Go (38) and Zen (71) model allowlists against the live OpenCode `/models` catalogs; removed stale `hy3-free` and `laguna-s-2.1-free`; labeled new free models (`ling-3.0-flash-fin-free`, `muse-spark-1.2-contributor-free`, `muse-spark-1.3-contributor-free`).
= 0.1.2 =
...
= 0.1.3 =
Model allowlists refreshed (Go: 38, Zen: 71). No action required.
= 0.1.2 =
